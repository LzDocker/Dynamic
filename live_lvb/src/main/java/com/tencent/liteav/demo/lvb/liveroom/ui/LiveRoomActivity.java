...
import com.tencent.liteav.demo.lvb.GenerateTestUserSig;
...
//                        + ", mode = " + mAudioManager.getMode(), Toast.LENGTH_SHORT).show();
                ...
                runOnUiThread(() -> {
                    try {
                        ...
                        if (resp.code != 0) {
                            ...
                            SharedPreferences sp1 = getSharedPreferences("com.tencent.demo", Context.MODE_PRIVATE);
    ...
    private void internalInitializeLiveRoom(LoginInfoResponse resp) {
        ...
        loginInfo.sdkAppID = GenerateTestUserSig.SDKAPPID;/*resp.sdkAppID*/
        ;
        loginInfo.userID = /*resp.userID*/ "1234567";
        loginInfo.userSig = "eJwtzF0LgjAYhuH-stNC5r5sQkdDIphBaCedVZvxZn40bQTRf0*mh8-1wP1FpS4ibx1KEYkwWocNxrYjVBA4JpRxkSzXYOpL34NBacwwppJsKJ0f**nB2ck55wRjPOsITTBJGZEyZksF7lP5KVpSSX3bq5PIzmp4dXBoVuLqjvmuzGndeZ*8H0pnRb1Fvz-nWzDR"; ///*resp.userSig*/GenerateTestUserSig.genTestUserSig(loginInfo.userID);
        loginInfo.userName = userName;
        loginInfo.userAvatar = userAvatar;
        LiveRoomActivity.this.userId = loginInfo.userID;
    ...
    private final class MLVBLiveRoomListener implements IMLVBLiveRoomListener {
         ...
         * <p>
         * 主播收到其他房间主播的 PK 请求
         * 如果同意 PK ，您需要调用 {@link MLVBLiveRoom#
         * startRemoteView(AnchorInfo, TXCloudVideoView, PlayCallback)}  接口播放邀约主播的流
         *
